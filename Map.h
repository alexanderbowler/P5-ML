...
public:
  ...
  Iterator find(const Key_type& k) const{
    Value_type dummyV = Value_type();
    Pair_type searchPair = std::pair<Key_type,Value_type>(k,dummyV);
    return tree.find(searchPair);
  }
  ...
  Value_type& operator[](const Key_type& k){
    Pair_type Search = {k,Value_type()};
    std::pair<Iterator,bool> found = insert(Search);
  ...
  std::pair<Iterator, bool> insert(const Pair_type &val){
    ...
    if(found!=end()){
      ...
      return std::pair<Iterator,bool>(tree.insert(val),true);
    }
  }
