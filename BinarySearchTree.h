...
public:
  ...
  BinarySearchTree(const BinarySearchTree &other)
    : root(copy_nodes_impl(other.root)), less(other.less) { }
  ...
  int tree_sway(){
    return tree_sway_impl(root);
  }

  int tree_sway_impl(Node * node){ 
...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static Node *copy_nodes_impl(Node *node) {
    ...
    add->left = copy_nodes_impl(node->left);
    add->right = copy_nodes_impl(node->right);
    return add;
  }
  ...
  static Node * find_impl(Node *node, const T &query, Compare less) {
    if(node==nullptr)
      return nullptr;
    if(!less(node->datum, query)&& !less(query, node->datum))  //work on this more
      return node;
  ...
  Compare less, const T* min, const T* max){
    if(empty_impl(node))
      return true;
    else if(((min!=nullptr)&&(!less(*min,node->datum)))
    ||((max!=nullptr)&&(!less(node->datum,*max)))){
      return false;
    }
    else
    return check_sorting_helper(node->right,less,&(node->datum),max)&&
              check_sorting_helper(node->left,less,min,&(node->datum));
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    return check_sorting_helper(node,less,nullptr,nullptr);
  }
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
    ...
    else if((!empty_impl(node->left))&&(less(val,max_element_impl(node->left)->datum)))
